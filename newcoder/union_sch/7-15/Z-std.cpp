...
void solve() {
    ...
    i64 mx_id = upper_bound(all(b), mid) - b.begin();
    ...
    while (qq--) {
        ...
        // 并更新优先队列q和bgbg
        // 我需要求当前的mid为第一个不崩溃的人,也就是当mi_num + mid_num >= (n+1LL)/2
        // 的时候,又因为前面的(n+1LL) / 2 是 从零开始计算的所以还要+1LL
        while (mi_num + mid_num - 1LL < (n + 1LL) / 2) {
            while (!q.empty() && bgbg.count(q.top())) {
                erase_bg(q.top());
                q.pop();
            }
...
// 2025.07.15——16:28:16 wa
/*错误样例
10 3
98 961 563 788 44 726 903 160 203 282
4 505
3 992
8 566
*/
...
/*错误样例
10 4
882 887 411 877 823 466 199 401 422 315
9 478
5 380
7 890
8 481
...
/*错误样例:
10 13
569 774 814 752 223 490 513 80 118 242
7 764
5 427
10 532
7 114
